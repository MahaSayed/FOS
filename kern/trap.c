...
void idt_init(void) {
	...
	gdt[GD_TSS >> 3] = SEG16(STS_T32A, (uint32)(&ts), sizeof(struct Taskstate),
			0);
	gdt[GD_TSS >> 3].sd_s = 0;
...
void trap(struct Trapframe *tf) {
	...
	if (tf->tf_trapno == IRQ0_Clock) {
		...
		if (bypassInstrLength != 0) {
			if (userTrap) {
				curenv->env_tf.tf_eip = (uint32*) ((uint32)(
						curenv->env_tf.tf_eip) + bypassInstrLength);
				env_run (curenv);
			} else {
				tf->tf_eip =
						(uint32*) ((uint32)(tf->tf_eip) + bypassInstrLength);
				kclock_resume();
				env_pop_tf(tf);
			}
		}
...
void placement(struct Env * curenv, uint32 fault_va, uint32 replacement_index,uint32 replacement)
{
	...
	if (page_permissions & PERM_BUFFERED)
	{
		...
		struct Frame_Info *frame_info = get_frame_info(curenv->env_page_directory,(void*) fault_va, &page_table);
	...
	{
		struct Frame_Info *new_frame_info;
		int ret = allocate_frame(&new_frame_info);
		if (ret != E_NO_MEM)
		{
			page_permissions |= (PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
			ret = map_frame(curenv->env_page_directory, new_frame_info,(void*) fault_va, page_permissions);
			if (ret != E_NO_MEM)
			{
				...
				if (ret == E_PAGE_NOT_EXIST_IN_PF)
				{
					if (fault_va > USTACKBOTTOM && fault_va <= USTACKTOP)
						...
						return panic("WRONG VIRTUAL ADDRESS");
				}
			}
			else
				return panic("MEMORY FULL");
		}
		else
			return panic("MEMORY FULL");
	}
	if (replacement == 0)
	{
		...
		env_page_ws_set_entry(curenv, replacement_index, ROUNDDOWN(fault_va, PAGE_SIZE));
}
...
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va) {
	...
	else
	{
		...
		if(isPageReplacmentAlgorithmModifiedCLOCK())
		{
			...
			{
				uint32 working_set_size = curenv->page_WS_max_size;
				uint32 starting_index = curenv->page_last_WS_index;
				uint32 i = starting_index;
				while (working_set_size > 0)
				{
					i = (i % curenv->page_WS_max_size);
					uint32 temp_VA = env_page_ws_get_virtual_address(curenv, i);
		...
		pt_set_page_permissions(curenv, victim_VA, PERM_BUFFERED, PERM_PRESENT);

		//check permissions of victim page
		uint32 page_permissions = pt_get_page_permissions(curenv, victim_VA);
		if (page_permissions & PERM_MODIFIED)
		{
